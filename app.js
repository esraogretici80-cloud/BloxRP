...
async function toggleBan(remove=false) {
  ...
  try {
    if (state.demo) {
      state.banned=remove?null:{uid,reason:$("banReason")?.value.trim(),expiresAt:Number($("banMinutes")?.value \vert{}\vert{} 0)?now()+Number($("banMinutes")?.value)*60000:0};
      renderBan();
    } else if (remove) {
...
async function signInWithGoogle() {
  if(!state.auth) { toast("Önce Firebase ayarlarını bağla.","bad"); openModal("setupModal"); return; }
  const btn=$("googleLoginBtn");
  try {
    if(btn){btn.disabled=true;btn.innerHTML='<span class="spinner"></span> Google bağlantısı açılıyor...';}
    ...
    if(btn){btn.disabled=false;btn.innerHTML='<span class="google-g">G</span> Google ile giriş yap';}
    toast(readableError(err),"bad");
...
async function boot() {
  ...
  renderSound(); renderRanks(); populateRankSelect(); bindUI();
  state.profile=loadLocalProfile(); renderProfile(); renderBan();
  const raw=localStorage.getItem(APP.storageKey);
  if(raw){
    try{
      setBoot("Kayıtlı Firebase ayarı bulundu...");
      await initFirebase(JSON.parse(raw));
    }catch(e){
      toast("Kayıtlı Firebase ayarıyla bağlantı kurulamadı. Ayarları kontrol et.","bad");
      if ($("setupError")) { $("setupError").textContent=readableError(e);$("setupError").classList.remove("hidden"); }
      openModal("setupModal");
      hideBootScreen();
    }
  } else {
    setBoot("Firebase ayarı bekleniyor...");
    openModal("setupModal");
    hideBootScreen();
  }
}
